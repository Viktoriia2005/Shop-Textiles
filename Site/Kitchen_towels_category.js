document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const res = await fetch("http://localhost:5000/products/tag/кухонні");
    const products = await res.json();

    if (!Array.isArray(products) || products.length === 0) {
      container.innerHTML = `<p class="text-center">Наразі немає кухонних рушників у наявності</p>`;
      return;
    }
    ...
    console.error("❌ Помилка завантаження кухонних рушників:", error);
