document.addEventListener("DOMContentLoaded", async () => {
  const container = document.getElementById("pillowsList");

  if (!container) {
    console.warn("⚠️ Елемент #pillowsList не знайдено");
    return;
  }
  ...
  try {
    ...
    if (!Array.isArray(products) || products.length === 0) {
      container.innerHTML = `<p class="text-center">Наразі немає декоративних подушок у наявності</p>`;
      return;
    }

    products.forEach(product => {
      const photoArray = product.Photo.split(",").map(p => p.trim()).filter(p => p.length > 0);
    ...
    console.error("❌ Помилка завантаження подушок:", error);
