...
// 🔹 Мапа категорій
const categoryMap = {
  blanketList: 1,   // Ковдри
  pillowsList: 2,   // Подушки
  veilList: 3,      // Покривала
  linensList: 4,    // Постільна білизна
  saunaList: 5,     // Рушники для бані
  kitchenList: 6,   // Кухонні рушники
  tableList: 7,     // Скатертини
  dressList: 8      // Халати
};
  ...
  searchInput.addEventListener("input", async function () {
    ...
    try {
      ...
      hints.forEach(item => {
        ...
        // 🔗 одразу редирект на сторінку товару
        option.addEventListener("click", () => {
          if (labelId) {
            window.location.href = `Product.html?id=${labelId}`;
          } else {
            // fallback: якщо немає id, робимо пошук
  ...
  searchInput.addEventListener("input", async function () {
    ...
    if (hint.length < 1) {
      ...
      try {
        ...
        feedback.innerHTML = `<p class="text-danger">Не вдалося завантажити товари</p>`;
      }
      return;
    }

    // 🔍 Основний пошук (залишаємо без змін)
    const form = document.getElementById("searchForm");
...
// 🛠️ Допоміжна функція для рендерингу карток товарів
