document.addEventListener("DOMContentLoaded", async () => {
  const container = document.getElementById("saunaList");
  ...
  if (!container) {
    console.warn("Element #saunaList not found");
    return;
  }
  ...
  const renderProducts = (products, page = 1) => {
    ...
    if (!Array.isArray(products) || products.length === 0) {
      container.innerHTML = `<p class="text-center">Наразі немає банних рушників у наявності</p>`;
  ...
  try {
    const res = await fetch(`${API_BASE}/products/category/5`);
    ...
    console.error("Помилка завантаження банних рушників:", error);
