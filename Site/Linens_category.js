document.addEventListener("DOMContentLoaded", async () => {
  ...
  const container = document.getElementById("linensList");

  try {
    const res = await fetch("http://localhost:5000/products/tag/Постіль");
    const products = await res.json();

    if (!Array.isArray(products) || products.length === 0) {
      container.innerHTML = `<p class="text-center">Наразі немає постільної білизни у наявності</p>`;
      return;
    }
    ...
    console.error("❌ Помилка завантаження постільної білизни:", error);
